...
description: Use when making strategic or architectural decisions, evaluating policy changes, designing features that change user behavior, or when the obvious solution feels too easy
---

# Second-Order Thinking

## Overview
Second-order thinking moves beyond immediate effects to consider what happens next, and what that leads to. First-order thinking is simplistic ("This action solves the problem"); second-order thinking asks "And then what?" repeatedly.
...
|-----------|-------------|--------------|
| Team is slow | Add more engineers | More engineers → more coordination → slower decisions |
| Users complain | Add the feature they request | Feature → complexity → more support load → less time for core work |
| Costs too high | Cut spending | Cuts → reduced quality → customer churn → revenue drop |
| Bug in prod | Hotfix immediately | Hotfix → skip testing → more bugs → trust erosion |
