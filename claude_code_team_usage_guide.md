...
- Guide 1 environment variable configuration changes significantly: `ANTHROPIC_BASE_URL`, `ANTHROPIC_API_KEY`, `ANTHROPIC_DEFAULT_OPUS_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_HAIKU_MODEL` all change. A migration checklist will be published as a Guide 1 amendment when migration is confirmed.
...
- The model selection guidance in Guide 1 changes: `ANTHROPIC_DEFAULT_OPUS_MODEL` becomes a native Claude Opus model, `ANTHROPIC_DEFAULT_SONNET_MODEL` becomes a native Claude Sonnet model, `ANTHROPIC_DEFAULT_HAIKU_MODEL` becomes a native Claude Haiku model. The pricing and tier analysis in Guide 1 is replaced with subscription billing instructions.
